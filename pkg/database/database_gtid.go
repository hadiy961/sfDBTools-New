// File : pkg/database/database_gtid.go
// Deskripsi : Fungsi untuk mengambil informasi GTID dari MariaDB/MySQL
...
func (c *Client) GetMasterStatus(ctx context.Context) (*GTIDInfo, error) {
...
func (c *Client) GetBinlogGTIDPos(ctx context.Context, binlogFile string, binlogPos int64) (string, error) {
	query := fmt.Sprintf("SELECT BINLOG_GTID_POS('%s', %d)", binlogFile, binlogPos)

	var gtidPos sql.NullString
	err := c.db.QueryRowContext(ctx, query).Scan(&gtidPos)
...
func (c *Client) GetFullGTIDInfo(ctx context.Context) (*GTIDInfo, error) {
	// Dapatkan master status terlebih dahulu
	gtidInfo, err := c.GetMasterStatus(ctx)
	if err != nil {
		return nil, err
	}

	// Dapatkan GTID position dari binlog
	gtidPos, err := c.GetBinlogGTIDPos(ctx, gtidInfo.MasterLogFile, gtidInfo.MasterLogPos)
	if err != nil {
