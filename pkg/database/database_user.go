...
func (c *Client) ExportUserGrantsForDatabases(ctx context.Context, databases []string) (string, error) {
	if len(databases) == 0 {
		return "", fmt.Errorf("daftar database kosong")
	}

	// Dapatkan daftar user
	allUsers, err := c.GetUserList(ctx)
	if err != nil {
		return "", err
	}
	...
	for _, userInfo := range allUsers {
		allGrants, err := c.GetUserGrants(ctx, userInfo.User, userInfo.Host)
		if err != nil {
			builder.WriteString(fmt.Sprintf("-- Warning: Failed to get grants for '%s'@'%s': %v\n",
				userInfo.User, userInfo.Host, err))
			continue
