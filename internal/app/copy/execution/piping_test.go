	...
	tests := []struct {
	...
	}{
		{
			...
			verify: func(t *testing.T, res string) {
				if strings.Contains(res, "`db_prod`.") { t.Error("Failed to replace source DB name with backticks") }
				if !strings.Contains(res, "`db_dev`.") { t.Error("Missing target DB name with backticks") }
			},
		},
		{
			...
			verify: func(t *testing.T, res string) {
				if strings.Contains(res, " db_prod.") { t.Error("Failed to replace source DB name with space prefix") }
				if !strings.Contains(res, " db_dev.") { t.Error("Missing target DB name with space prefix") }
			},
		},
		{
			...
			verify: func(t *testing.T, res string) {
				if strings.Contains(res, "DEFINER") { t.Error("Failed to strip DEFINER comment style") }
			},
		},
		{
			...
			verify: func(t *testing.T, res string) {
				if strings.Contains(res, "DEFINER") { t.Error("Failed to strip DEFINER direct style") }
			},
		},
		{
...
func TestTransformSQLStream_TableDetection(t *testing.T) {
	...
	if !found["t_users"] { t.Error("Failed to detect structure table 't_users'") }
	if !found["t_logs"] { t.Error("Failed to detect data table 't_logs'") }
}
