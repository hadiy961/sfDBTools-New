...
func (s *Service) backupDatabaseGeneric(ctx context.Context, mode string, dbName string, dbList []string, backupOpts *restoremodel.RestoreBackupOptions) (string, error) {
	...
	if mode == consts.ModeCombined {
		userDefPath, userGrantsPath, ugErr := backupSvc.ExportUserGrantsIfNeeded(ctx, outputPath, dbList, false)
		if ugErr != nil {
			s.Log.Warnf("Gagal export user grants (best-effort): %v", ugErr)
		}
		permissions := s.Config.Backup.Output.MetadataPermissions
		backupSvc.UpdateMetadataUserGrantsPath(outputPath, userDefPath, userGrantsPath, permissions)
	}
