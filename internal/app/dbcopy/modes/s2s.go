// File : internal/app/dbcopy/modes/s2s.go
// Deskripsi : S2S (Secondary to Secondary) executor
...
// S2SExecutor implements Executor interface untuk S2S copy
type S2SExecutor struct {
	log  applog.Logger
	svc  CopyService
	opts *model.S2SOptions
}

// NewS2SExecutor membuat instance baru S2S executor
func NewS2SExecutor(log applog.Logger, svc CopyService, opts *model.S2SOptions) *S2SExecutor {
	return &S2SExecutor{
		log:  log,
		svc:  svc,
		opts: opts,
...
// Execute menjalankan S2S copy operation
func (e *S2SExecutor) Execute(ctx context.Context) (*model.CopyResult, error) {
	...
	if err := e.svc.ValidateNotCopyToSelf(srcProfile, tgtProfile, sourceDB, targetDB, string(model.ModeS2S)); err != nil {
		return nil, err
	}
	...
	if e.opts.DryRun {
		e.log.Infof("[DRY-RUN] S2S Copy Plan:")
	...
	// Backup source database (S2S backup secondary database, gunakan ModeSecondary untuk konsistensi logging)
	srcDump, err := e.svc.BackupDB(ctx, srcProfile, srcClient, sourceDB, e.opts.Ticket, workdir, e.opts.ExcludeData, consts.ModeSecondary)
	if err != nil {
	...
	// Jika eksplisit mode, gunakan restore single
	if e.opts.SourceDB != "" && e.opts.TargetDB != "" {
		...
		err = e.svc.RestoreSecondary(ctx, tgtProfile, srcDump, dmartDump, e.opts.Ticket, e.opts.ClientCode, e.opts.TargetInstance, encKey, e.opts.IncludeDmart, true, !e.opts.PrebackupTarget, e.opts.ContinueOnError, DetermineNonInteractiveMode(&e.opts.CommonCopyOptions))
		if err != nil {
			result.Error = err
			return result, fmt.Errorf("gagal restore ke target secondary database (client=%s instance=%s): %w", e.opts.ClientCode, e.opts.TargetInstance, err)
		}
	}

	result.Success = true
	result.Message = fmt.Sprintf("S2S copy berhasil: %s → %s", sourceDB, targetDB)
	return result, nil
}

func (e *S2SExecutor) resolveDatabaseNames(ctx context.Context, srcClient *database.Client) (sourceDB, targetDB string, err error) {
	if e.opts.SourceDB != "" && e.opts.TargetDB != "" {
	...
	sourceDB = naming.BuildSecondaryDBName(primary, e.opts.SourceInstance)
	targetDB = naming.BuildSecondaryDBName(primary, e.opts.TargetInstance)
