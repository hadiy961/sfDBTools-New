...
restore_full() {
    database_find="$(find /media/dbtosecondary-nbc/sf7/production/${year}${month}${day} -maxdepth 1 -type f -name "\\[FULL\\]_dbsf_*${account_code}_[0-9]*.sql.enc" | sort -n | tail -n 1 2>/dev/null)"
    grant_find="$(find /media/dbtosecondary-nbc/sf7/production/${year}${month}${day} -maxdepth 1 -type f -name "grant_${account_code}.sql" | sort -n | tail -n 1 2>/dev/null)"
    if [[ ! -f "$database_find" ]]
    then
        echo "Backup file not found";
        exit;
    fi
    databases[0]="${database_find:56:-24}_${secondary}_dmart"
    databases[1]="${database_find:56:-24}_${secondary}"
    ...
    do
        ...
        mysql -u"$user" -p"$password" -h"$host" -P"$port" -sNe "${grant/database/${databases[1]}_temp}"
    done < "$grant_find"
    for database in ${databases[@]}
    do
        echo ""
        file_name_find="$(find /media/dbtosecondary-nbc/sf7/production/${year}${month}${day} -maxdepth 1 -type f -name "\\[FULL\\]_${database//"_$secondary"/}_[0-9]*.sql.enc" 2>/dev/null)"
    ...
    mysql -u"$user" -p"$password" -h"$host" -P"$port" -sNe "SET GLOBAL max_statement_time=0;" && mysql -u"$user" -p"$password" -h"$host" -P"$port" -sNe "DROP DATABASE IF EXISTS ${databases[1]}_temp; CREATE DATABASE IF NOT EXISTS ${databases[1]}_temp" && mysql -u"$user" -p"$password" -h"$host" -P"$port" -sNe "SET GLOBAL max_statement_time=60;"
}

clear
echo "RESTORE DATABASE FROM PRODUCTION (CUSTOM DATE & TIME)"
echo ""
echo "1. Use Full Backup"
echo "2. Use Incremental Backup"
echo ""
echo "0. Exit"
read -p "Select an option: " option_backup
case "$option_backup" in
0)
    exit;
    ;;
1|2)
