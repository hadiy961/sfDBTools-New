...
restore() {
    database_find="$(find /media/dbtosecondary-nbc/sf7/production/${year}${month}${day} -maxdepth 1 -type f -name "dbsf_*${account_code}_${year}${month}${day}_[0-9]*.sql.gz" | sort -n | tail -n 1 2>/dev/null)"
    ...
    databases[0]="${database_find:49:-23}_${secondary}_dmart"
    databases[1]="${database_find:49:-23}_${secondary}"
    ...
    do
        echo ""
        file_name_find="$(find /media/dbtosecondary-nbc/sf7/production/${year}${month}${day} -maxdepth 1 -type f -name "${database//"_$secondary"/}_${year}${month}${day}_[0-9]*.sql.gz" 2>/dev/null)"
