...
host="172.17.71.139"
port="33304"
daily_backup_path="/media/daily_backup"
last_backup_path="/media/dbtosecondary-nbc/last_backup"
...
check() {
    ...
    echo ""
    file_name_find=$(find ${daily_backup_path}/*/*/*/${main_database_chosen}/ -type f -name "\\[FULL\\]_${main_database_chosen}_[0-9]*.sql.enc" 2>/dev/null | sort -n);
    if [[ $(echo "$file_name_find" | wc -l)>0 ]] && [[ -n "$file_name_find" ]]
    then
        echo "Available backup files of "${main_database_chosen#*_*_}":"
        ...
        then
            ...
            if [[ -f "$file_name_chosen" ]] || [[ -n "$file_name_chosen" ]]
            then
                dmart_database="${main_database_chosen}_dmart"
                temp_database="${main_database_chosen}_temp"
		dmart_file_name_chosen="$(echo "$file_name_chosen" | sed "s/$main_database_chosen/$dmart_database/g")"
                echo "Backup files of ${main_database_chosen#*_*_} on $(date +'%d %B %Y %H:%M:%S' -r "$file_name_chosen") have been found"
                ...
                case "$restore_confirmation" in
                    [Yy]|[Yy][Ee][Ss])
                        ...
                        if [[ -f "$dmart_file_name_chosen" ]] || [[ -n "$dmart_file_name_chosen" ]]
                        then
...
clear
echo "CHECK AND RESTORE DATABASE FROM DAILY BACKUP"
echo ""
