...
restore_full() {
    ...
    if [[ "$database" == *_dmart ]]; then
        file_name_find="$(find "/media/dbtolocal-nbc/sf7/production/${year}${month}${day}" -maxdepth 1 -type f -name "\\[FULL\\]_dbsf_*${account_code}_dmart_[0-9]*.sql.enc" 2>/dev/null)"
    else
        file_name_find="$(find "/media/dbtolocal-nbc/sf7/production/${year}${month}${day}" -maxdepth 1 -type f -name "\\[FULL\\]_dbsf_*${account_code}_[0-9]*.sql.enc" 2>/dev/null)"
    fi
    echo ""
...
case "$option_backup" in
1)
    restore_full 2>&1 "$dmart_database" | tee -a "$log"
    restore_full 2>&1 "$main_database" | tee -a "$log"
    ;;
2)
    exit;
    ;;
