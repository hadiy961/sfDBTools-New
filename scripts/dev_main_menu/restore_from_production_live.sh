...
restore() {
    ...
    if [[ "$database" == *_dmart ]]; then
        file_name_find="$(find "/media/dbtolocal-nbc/sf7/production/${year}${month}${day}" -maxdepth 1 -type f -name "dbsf_*${account_code}_dmart_${year}${month}${day}_[0-9]*.sql.gz" 2>/dev/null)"
    else
        file_name_find="$(find "/media/dbtolocal-nbc/sf7/production/${year}${month}${day}" -maxdepth 1 -type f -name "dbsf_*${account_code}_${year}${month}${day}_[0-9]*.sql.gz" 2>/dev/null)"
    fi
    echo ""
