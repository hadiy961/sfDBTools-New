...
archive() {
    ...
    do
        ...
        then
            find /media/BACKUPDBDEV/archive_backup/${archived_date:0:4}${archived_date:5:2}${archived_date:8:2}/ -type f -name "${database}_${archived_date:0:4}${archived_date:5:2}${archived_date:8:2}_[0-9]*.sql.gz" -exec rm {} \;
        fi
