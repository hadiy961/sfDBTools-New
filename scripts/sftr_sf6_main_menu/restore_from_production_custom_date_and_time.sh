...
restore_full() {
    database_find="$(find /media/dbtosecondary-nbc/sf6_gdpro/production/${year}${month}${day} -maxdepth 1 -type f -name "\\[FULL\\]_dbsf_*${account_code}_[0-9]*.sql.enc" | sort -n | tail -n 1 2>/dev/null)"
    ...
    databases[0]="${database_find:62:-24}_${secondary}_dmart"
    databases[1]="${database_find:62:-24}_${secondary}"
    ...
    do
        echo ""
        file_name_find="$(find /media/dbtosecondary-nbc/sf6_gdpro/production/${year}${month}${day} -maxdepth 1 -type f -name "\\[FULL\\]_${database//"_$secondary"/}_[0-9]*.sql.enc" 2>/dev/null)"
